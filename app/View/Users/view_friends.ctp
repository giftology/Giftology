    ...
    <div class="left-container">
            <?php if(isset($today_users) & $today_users['Reminders']): ?>
            <div>
                    <h4 class="line-header">Celebrate today<div class="calendar"><p>28</p></div></h4>
                    <?= $this->element('friend_list',
                                        array('reminders' => $today_users['Reminders'],
                                              'ocasion' => 'Birthday', ),
                                        array('cache' => array(
                                                'key' => $facebook_user.'today.Birthday'.date('Y-m-d')))); ?>
            </div>
            <div class="clear"></div>
            <?php endif; ?>

            <?php if(isset($this_month_users) & $this_month_users['Reminders']): ?>
            <div>
                    <h4 class="line-header">Celebrate this month </h4>
                    <?= $this->element('friend_list',
                                        array('reminders' => $this_month_users['Reminders'],
                                              'ocasion' => 'Birthday', ),
                                        array('cache' => array(
                                                'key' => $facebook_user.'Month.Birthday'.date('Y-m-d')))); ?>
            </div>
            <?php endif; ?>

            <?php if(isset($all_users) & $all_users['Reminders']): ?>
            <div>
                    <h4 class="line-header">Send a gift card to any friend </h4>
                    <?= $this->element('friend_list',
                                        array('reminders' => $all_users['Reminders']),
                                        array('cache' => array(
                                                'key' => $facebook_user.'All'.date('Y-m-d')))); ?>
            </div>
            <?php endif; ?>
    </div>
