...
/**
 * AesCrypt
 * Encrypt/Decrypt using AES-256bit Rijndael 
 * URL-safe
 *
 * CakePHP adaptation
 ...
 */
class AesCryptComponent extends Component {
	...
	public static function encrypt($data) {
		$salt = self::__getSecuritySalt();
		$data = serialize($data);
		$data = MT_AesCtr::encrypt($data, $salt, 256);
	...
	public static function decrypt($data) {
		$salt = self::__getSecuritySalt();
		$data = strtr($data, '-_', '+/');
		$data = MT_AesCtr::decrypt($data, $salt, 256);
	 ...
	 * @return string|mixed
	 */
	public static function decryptStr($data) {
		$salt = self::__getSecuritySalt();
		$data = strtr($data, '-_', '+/');
		$data = MT_AesCtr::decrypt($data, $salt, 256);
		return $data;
	...
	/**
	 * Get security salt
	 * CakePHP's Security.salt is used
	 * 
	 * @return string
	 */
	private static function __getSecuritySalt() {
		$salt = Configure::read('Security.salt');
		if (empty($salt)) {
			trigger_error('Security salt is empty', E_USER_ERROR);
			return false;
		}
		return $salt;
	}
 ...
 * 
 * class "Aes" renamed to "MT_Aes"
 * class "AesCtr" renamed to "MT_AesCtr"
 * 
...
class MT_AesCtr extends MT_Aes {
  ...
  public static function encrypt($plaintext, $password, $nBits) {
    ...
    $key = MT_Aes::cipher($pwBytes, MT_Aes::keyExpansion($pwBytes));
    ...
    $keySchedule = MT_Aes::keyExpansion($key);
    //print_r($keySchedule);
    ...
    for ($b=0; $b<$blockCount; $b++) {
      ...
      $cipherCntr = MT_Aes::cipher($counterBlock, $keySchedule);  // -- encrypt counter block --
