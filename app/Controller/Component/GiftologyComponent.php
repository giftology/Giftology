...
class GiftologyComponent extends Component {
	...
	function postToFB($sender_fb_id, $receiver_fb_id, $access_token, $url, $message, $post_to_fb = null) {
		...
		curl_setopt($ch,CURLOPT_URL,'https://graph.facebook.com/'.$sender_fb_id.'/giftology:send');
	...
	function welcomePostToFB($sender_fb_id, $access_token, $url, $message) {
		...
		$fields = array(
			'access_token'=> $access_token,
	        'message'=>$message,
	        'giftology'=>$url.'/users/login/?visit=1'
	    );
